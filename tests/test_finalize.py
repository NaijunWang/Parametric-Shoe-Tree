...
from custom_shoe_tree.finalize import run_best
...
def test_finalize_cli_writes_phase5_artifacts(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "custom_shoe_tree.cli",
            "finalize",
            str(phase3_output_dir / "shoe_tree_warp.obj"),
            "-o",
            str(output_dir),
        ],
        cwd=project_root,
    ...
    assert (output_dir / "shoe_tree_0014-B.obj").exists()
    assert (output_dir / "shoe_tree_0014-B.stl").exists()
