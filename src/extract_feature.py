...
def align_mesh_via_pca(mesh):
    ...
    def get_max_z_span(m, y_range):
        v_subset = m.vertices[(m.vertices[:, 1] > y_range[0]) & (m.vertices[:, 1] < y_range[1])]
...
def main():
    parser = argparse.ArgumentParser(description="Foot Feature Extractor - Exhibition Version")
    ...
    print(f"\n[Section Features]")
    if sec_ball:
        print(f"1. Toe-Instep Perimeter (Red):    {sec_ball.length:.2f} mm (at Y={ball_y:.1f})")
    if sec_instep:
        print(f"2. Instep Peak Perimeter (Yellow): {sec_instep.length:.2f} mm (at Y={instep_y:.1f})")
    print("-" * 50)
    ...
    if not args.no_show:
        ...
        if sec_ball and len(sec_ball.entities) > 0:
            ...
            p_ball = trimesh.creation.box(extents=[dims['width']*1.5, 0.2, dims['height']*1.2])
        ...
        if sec_instep and len(sec_instep.entities) > 0:
            sec_instep.colors = np.tile([255, 255, 0, 255], (len(sec_instep.entities), 1))
            p_instep = trimesh.creation.box(extents=[dims['width']*1.5, 0.2, dims['height']*1.2])
